...
def draw_page(request):
    ...
    if DEFAULT_MODEL_FILENAME not in model_files:
        model_files.insert(0, DEFAULT_MODEL_FILENAME)
...
def extract_and_resize_parts(base64_img, size=(128, 128), rotation_prob=0.3):
    ...
    all_images = [np.expand_dims(img, axis=(0, -1)) / 255.0 for img in all_images]
    return all_images
...
def filter_predictions_by_hints(candidates, target_word, hints):
    ...
    length_filtered = [c for c in candidates if len(normalize_word(c)) == len(target_normalized)]
    ...
    for hint in hints:
        ...
        for candidate in hint_filtered:
            ...
            if len(candidate_normalized) > hint_index and candidate_normalized[hint_index] == hint_letter:
...
def predict(request):
    ...
    try:
        ...
        # ✅ Read session BEFORE spawning threads — avoids race conditions
        previous_failed = list(request.session.get('previous_failed', []))
        previous_failed_set = set(previous_failed)
        ...
        def process_model(model, model_idx):
            ...
            for img_tensor in processed_images:
                ...
                image_b64_list.append(image_to_base64(img_arr))
            ...
            # Uses previous_failed from outer scope (read before threads — thread-safe)
            result = []
            for cls in filtered:
            ...
            result = [
                (name, prob) for name, prob in result
            ...
            return [
                ...
                inference_time,
                model_idx,
                [{'img': b64, 'preds': preds} for b64, preds in zip(image_b64_list, visual_preds)]
            ]
        ...
        # 🚀 Run all models IN PARALLEL — results arrive as each model finishes
        with ThreadPoolExecutor() as executor:
            ...
            for future in as_completed(future_to_model):
                try:
                    ...
                    # Chance to return early with the first model that finishes
                    ...
                    draw_print(f"❌ Error getting result from model {future_to_model[future]}: {ex}")
        ...
        # ✅ Update session AFTER all threads finish — safe
        if not any_correct:
